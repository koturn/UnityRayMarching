        ...
        Pass
        {
            ...
            float sdf(float3 p, out float3 color)
            {
                static const float3 kColors[6] = {
                    float3(0.8, 0.4, 0.4),  // R
                    float3(0.8, 0.8, 0.4),  // Y
                    float3(0.4, 0.8, 0.4),  // G
                    float3(0.4, 0.8, 0.8),  // C
                    float3(0.4, 0.4, 0.8),  // B
                    float3(0.8, 0.4, 0.8)   // M
                };
                static const float kOneSixthPi = UNITY_PI / 3.0;
                static const float kInvOneSixthPi = 1.0 / kOneSixthPi;

                const float radius = _TorusRadius + _SinTime.w * _TorusRadiusAmp;

                float d = sdTorus(p, float2(radius, _TorusWidth));
                color = float3(1.0, 0.75, 0.25);

                p.xy = rotate2D(p.xy, _Time.y);
                const float xyAngle = atan2(p.y, p.x);
                const float rotUnit = floor(xyAngle * kInvOneSixthPi);
                p.xy = rotate2D(p.xy, kOneSixthPi * rotUnit + kOneSixthPi / 2.0);
